...
RSpec.describe AsqsController, type: :controller do
  ...
  describe '::index' do
    ...
    it 'sorts asqs in proper order' do
      ...
      get :index
      expect(assigns(:asqs).first.name).to eq('one')
      expect(assigns(:asqs).second.name).to eq('two')
      expect(assigns(:asqs).third.name).to eq('three')
      expect(assigns(:asqs).fourth.name).to eq('four')
      expect(assigns(:asqs).fifth.name).to eq('five')
  ...
  describe '.activity_rows' do
    ...
    it 'paginates correctly with last_id' do
      ...
      get :activity_rows, id: a.id, last_id: 91
      expect(assigns(:activities).first.detail).to eq('activity 90')
    ...
    it 'orders activity correctly' do
      ...
      get :activity_rows, id: a.id
      expect(assigns(:activities).first.detail).to eq('activity 10')
    ...
    it 'paginates at 75' do
      ...
      get :activity_rows, id: a.id
      expect(assigns(:activities).first.detail).to eq('activity 100')
    ...
    it 'returns 404 when no items remain' do
      ...
      expect { get :activity_rows, id: a.id, last_id: a.activities.first.id }
    ...
    describe 'Asq status for Nagios' do
      it 'returns clear_new in JSON format' do
        ...
        get :show, format: :json, id: asq.id
